...
public class HosomakiFragment extends BaseFragment {

    // For debugging purposes
    private static final String TAG = "HosomakiFragment";
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View rootView = inflater.inflate(R.layout.fragment_hosomaki, container, false);
        rootView.setTag(TAG);

        recyclerView = (RecyclerView) rootView.findViewById(R.id.recyclerViewHosomaki);
        ...
        for (int i = 0; i < app.dataHosomaki.size(); i++) {
            adapter.addItem(app.dataHosomaki.get(i));
        }
    ...
    public class CustomDataAdapter extends RecyclerView.Adapter<DataListViewHolder> {
        ...
        public DataListViewHolder onCreateViewHolder(ViewGroup parent, final int viewType) {
            View view = getActivity().getLayoutInflater().inflate(R.layout.fragment_hosomaki_item, parent, false);

            view.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    int id = recyclerView.getChildLayoutPosition(view);
                    String category = app.dataHosomaki.get(id).getCategory();
                    checkForAllergies(view);
                    Intent myintent=new Intent(getContext(), SpecificDishActivity.class);
                    myintent.putExtra("Category", category);
                    myintent.putExtra("hosomakiID", id);
        ...
        public void onBindViewHolder(DataListViewHolder holder, int position) {
            Item item = app.dataHosomaki.get(position);
    ...
    private class DataListViewHolder extends RecyclerView.ViewHolder {
        ...
        public DataListViewHolder(final View itemView) {
            super(itemView);
            title = itemView.findViewById(R.id.hosomaki_item_name);
            pcs = itemView.findViewById(R.id.hosomaki_item_pcs);
            price = itemView.findViewById(R.id.hosomaki_item_price);
            image = itemView.findViewById(R.id.hosomaki_item_image);
            addToBasket = itemView.findViewById(R.id.hosomaki_item_addToBasket);
            addToBasketFrame = itemView.findViewById(R.id.hosomaki_item_addToBasketFrame);

            addToBasket.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    int quantity = app.dataHosomaki.get(getAdapterPosition()).getQuantity();
                    int index = getAdapterPosition();
                    app.getCart().addItem(app.dataHosomaki.get(index));
                    quantity++;
                    app.dataHosomaki.get(index).setQuantity(quantity);
    ...
    public void checkForAllergies(View view) {
        ...
        while(i < allergies.getAllergies().size()) {
            try {
                ...
                App app = App.getInstance();
                inputStr = app.dataHosomaki.get(id).getAllergies().toLowerCase();
