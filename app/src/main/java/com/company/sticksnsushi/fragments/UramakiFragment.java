...
public class UramakiFragment extends BaseFragment {

    // For debugging purposes
    private static final String TAG = "UramakiFragment";
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View rootView = inflater.inflate(R.layout.fragment_uramaki, container, false);
        rootView.setTag(TAG);

        recyclerView = (RecyclerView) rootView.findViewById(R.id.recyclerViewUramaki);
        ...
        for (int i = 0; i < app.dataUramaki.size(); i++) {
            adapter.addItem(app.dataUramaki.get(i));
        }
    ...
    public class CustomDataAdapter extends RecyclerView.Adapter<DataListViewHolder> {
        ...
        public DataListViewHolder onCreateViewHolder(ViewGroup parent, final int viewType) {
            View view = getActivity().getLayoutInflater().inflate(R.layout.fragment_uramaki_item, parent, false);

            view.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    int id = recyclerView.getChildLayoutPosition(view);
                    String category = app.dataUramaki.get(id).getCategory();
                    checkForAllergies(view);
                    Intent myintent=new Intent(getContext(), SpecificDishActivity.class);
                    myintent.putExtra("Category", category);
                    myintent.putExtra("uramakiID", id);
        ...
        public void onBindViewHolder(DataListViewHolder holder, int position) {
            Item item = app.dataUramaki.get(position);
    ...
    private class DataListViewHolder extends RecyclerView.ViewHolder {
        ...
        public DataListViewHolder(final View itemView) {
            super(itemView);
            title = itemView.findViewById(R.id.uramaki_item_name);
            pcs = itemView.findViewById(R.id.uramaki_item_pcs);
            price = itemView.findViewById(R.id.uramaki_item_price);
            image = itemView.findViewById(R.id.uramaki_item_image);
            addToBasket = itemView.findViewById(R.id.uramaki_item_addToBasket);
            addToBasketFrame = itemView.findViewById(R.id.uramaki_item_addToBasketFrame);

            addToBasket.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    int quantity = app.dataUramaki.get(getAdapterPosition()).getQuantity();
                    int index = getAdapterPosition();
                    app.getCart().addItem(app.dataUramaki.get(index));
                    quantity++;
                    app.dataUramaki.get(index).setQuantity(quantity);
    ...
    public void checkForAllergies(View view) {
        ...
        while(i < allergies.getAllergies().size()) {
            try {
                ...
                App app = App.getInstance();
                inputStr = app.dataUramaki.get(id).getAllergies().toLowerCase();
